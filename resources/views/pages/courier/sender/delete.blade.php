...
<div class="card">
    <div class="card-header">
       <div class="row flex-between-center">
                <div class="col-4 col-sm-auto d-flex align-items-center pe-0">
                  <h5 class="fs-0 mb-0 text-nowrap py-2 py-xl-0">Sender Details</h5>
                </div>
   ...
   <table class="table table-bordered table-striped">
      <tr><th>Id</th><td> {{$sender->id}}</td></tr>
      <tr><th>Name</th><td> {{$sender->name}}</td></tr>      
      <tr><th>Contact</th><td> {{$sender->contact}}</td></tr>
      <tr><th>Address</th><td> {{$sender->address}}</td></tr>
     
      <tr>
        <td>
          <form action="{{url('senders/'.$sender->id)}}" method="post">
          @csrf
          @method("DELETE")          
